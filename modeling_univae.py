...
from vonMisesFisherSampling import vonMisesFisherSampling as vMF
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, history_states=None):
        # print("hidden states形状：", hidden_states.shape,"attation mask形状：", attention_mask.shape)
        # hidden states形状： torch.Size([64, 256, 768]) 
        # attation mask形状： torch.Size([64, 1, 256, 256])
        ...
        # print("mixed_qkv形状：", mixed_query_layer.shape, "qkv形状：", key_layer.shape)
        # mixed_qkv形状： torch.Size([64, 256, 768]) qkv形状： torch.Size([64, 12, 256, 64])

        attention_scores = torch.matmul(
            query_layer / math.sqrt(self.attention_head_size), key_layer.transpose(-1, -2))
        # print("attention score(mask) 的形状：", attention_scores.shape)
        # [bsz, num_head, seq_len, seq_len] [64, 12, 256, 256]
        ...
        # print("context layer的形状：", context_layer.shape) 
        # [bsz, head_num, seq_len, head_hidden]
        context_layer = context_layer.permute(0, 2, 1, 3).contiguous()
        # print("permuate后 context layer的形状：", context_layer.shape)
        # [bsz, seq_len, head_num, head_hidden]  [64, 256, 12, 64]
        new_context_layer_shape = context_layer.size()[
            :-2] + (self.all_head_size,)
        context_layer = context_layer.view(*new_context_layer_shape)
        # print("new_context_layer的形状：", context_layer.shape) # [bsz, seq_len, 768]
...
def get_vae_attention_mask(cur_layer, attention_mask):
    if cur_layer == None:
        return attention_mask
    # TODO 

class BertLayer(nn.Module):
    def __init__(self, config, cur_layer=None, kappa=0.4, latent_dim=32, topk=4):
        super(BertLayer, self).__init__()
        self.attention = BertAttention(config)
        self.intermediate = BertIntermediate(config)
        self.output = BertOutput(config)
        self.cur_layer = cur_layer # 用于 VAE 层次判断
        self.topk = topk
        self.total_layer = config.num_hidden_layers # 12 层
        self.vmf = vMF(kappa=kappa, dims=latent_dim)
        self.reconstruct_compress = nn.Linear(config.hidden_size, latent_dim)
        self.reconstruct_recover = nn.Linear(latent_dim, config.hidden_size)
        

    def forward(self, hidden_states, attention_mask, history_states=None):
        attention_mask = get_vae_attention_mask(self.cur_layer, attention_mask)
        attention_output = self.attention(
            hidden_states, attention_mask, history_states=history_states)
        intermediate_output = self.intermediate(attention_output)
        layer_output = self.output(intermediate_output, attention_output)
        # print("layer_output(每层输出的hidden_state)的形状：", layer_output.shape)
        # [bsz, seq_len, 768]

        # ****** 重构[CLS] ***** 
        if self.cur_layer and self.cur_layer > self.total_layer - self.topk:
            # 只有高层参与重构
            cls_vector = layer_output[:,0,:]
            comp_vector = self.reconstruct_compress(cls_vector)
            vmf_vector = self.vmf(comp_vector) # vMF 采样
            r_cls = self.reconstruct_recover(vmf_vector)
            layer_output[:,0,:] = r_cls
        # ****** 重构[CLS] ***** 
        return layer_output
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, prev_embedding=None, prev_encoded_layers=None):
        ...
        if not output_all_encoded_layers:
            all_encoder_layers.append(hidden_states)
        # print("last_hidden_state的形状：", hidden_states.shape) # [bsz, seq_len, 768]
        # print("all_encoder_layers(all_hidden_states)的长度：", len(all_encoder_layers))
...
class UnilmModel(UnilmPreTrainedModel): # BertModel
    def __init__(self, config):
    ...
    def get_extended_attention_mask(self, input_ids, token_type_ids, attention_mask):
        ...
        # print("input_ids的形状：", input_ids.shape) # [bsz, seq_len]
        ...
        # 上面是解决 使用多GPU训练时 stopIteration 的错误
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True):
        ...
        # print("extended_attention_mask的形状：", extended_attention_mask.shape)
        # torch.Size([64, 1, 256, 256]) 
        embedding_output = self.embeddings(
            input_ids, token_type_ids)
        encoded_layers = self.encoder(embedding_output, extended_attention_mask,
                                      output_all_encoded_layers=output_all_encoded_layers)
        # print("每层encoded_layers的形状：", encoded_layers[-1].shape) # [bsz, seq_len, 768]
        # print("all_encoded_layers的长度：", len(encoded_layers)) # 1 或者 12 还是 13？
        sequence_output = encoded_layers[-1] 
        pooled_output = self.pooler(sequence_output) # 
        # print("pooled_output的形状：", pooled_output.shape) # [bsz, 768]
        if not output_all_encoded_layers: 
            encoded_layers = encoded_layers[-1]
        return encoded_layers, pooled_output


class UnilmModelIncr(UnilmModel): # BertModelIncr
    def __init__(self, config):
...
class UnilmForVAE(UnilmPreTrainedModel):
...
class UnilmForVAEDecode(UnilmPreTrainedModel):
    def __init__(self, config, mask_word_id=0,
        ...
        super(UnilmForVAEDecode, self).__init__(config)
        self.bert = UnilmModelIncr(config)
        self.cls = BertOnlyMLMHead(config)
        self.crit_mask_lm = nn.CrossEntropyLoss(reduction='none')
        self.mask_word_id = mask_word_id # [MASK] 的 id
        self.search_beam_size = search_beam_size # 
        self.length_penalty = length_penalty
        self.eos_id = eos_id # "[SEP]", "[S2S_SOS]" 的 id
        self.sos_id = sos_id
        self.forbid_duplicate_ngrams = forbid_duplicate_ngrams
        self.forbid_ignore_set = forbid_ignore_set
        self.ngram_size = ngram_size # 3
    ...
    def forward(self, input_ids, token_type_ids, position_ids, attention_mask):
        # print(position_ids.shape == token_type_ids.shape) # True
        # print(position_ids[0]) # range(0, max_len)
        if self.search_beam_size > 1: 
            return self.beam_search(input_ids, token_type_ids, position_ids, attention_mask)

        input_shape = list(input_ids.size())
        # print("decode中输入的shape：", input_shape) # [bsz, max_a_len] # batch 的第一个句子的长度 
        batch_size = input_shape[0]
        input_length = input_shape[1]
        output_shape = list(token_type_ids.size())
        # print("decode中输出的shape：", output_shape) # [bsz, max_len_in_batch] # 两个句子的总长度
        output_length = output_shape[1] # max_len_in_batch

        output_ids = []
        prev_embedding = None
        prev_encoded_layers = None
        curr_ids = input_ids 
        mask_ids = input_ids.new(batch_size, 1).fill_(self.mask_word_id) # [bsz, 1] 全部都填入MASK
        next_pos = input_length # 从第一句结束处开始预测
        # print(list(curr_ids.size())[1] == next_pos) # True
        while next_pos < output_length: 
            curr_length = list(curr_ids.size())[1] # 输入a的长度
            start_pos = next_pos - curr_length # 循环第一次一定为 0 ，之后随着 next_pos 递增
            x_input_ids = torch.cat((curr_ids, mask_ids), dim=1) 
            # print(x_input_ids.shape, curr_ids.shape)
    ...
    def beam_search(self, input_ids, token_type_ids, position_ids, attention_mask):
        input_shape = list(input_ids.size())
        print("decode中输入的shape：", input_shape)
        batch_size = input_shape[0]
        input_length = input_shape[1]
        output_shape = list(token_type_ids.size())
        print("decode中输出的shape：", output_shape)
        ...
        while next_pos < output_length:
            ...
            def select_beam_items(x, ids):
                ...
                if id_rank < x_rank:
                    ...
                    ids = ids.expand(id_shape + x_shape[1:]).long()
        ...
        for b in range(batch_size):
            ...
            if frame_id == -1:
                ...
                for fid in range(frame_id, 0, -1):
                    # print("fid: {}, type: {}".format(fid, type(fid)))
                    pos_in_frame = int(ptrs[fid][pos_in_frame])
                    # print("pos_in_frame: {}, type: {}".format(pos_in_frame, type(pos_in_frame)))
