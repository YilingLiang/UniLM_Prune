...
def main():
    ...
    if args.do_train:
        print("Loading Train Dataset导入训练数据 ", args.data_dir)
        bi_uni_pipeline = [utils_seq2seq.Preprocess4AE(
                            args.max_pred, 
                            args.mask_prob, 
    ...
    model = model_class.from_pretrained( # UnilmForSeq2Seq
    ...
    if args.do_train:
        ...
        for i_epoch in trange(start_epoch, int(args.num_train_epochs)+1, desc="Epoch", disable=args.local_rank not in (-1, 0)):
            ...
            save_dir = args.output_dir + "/epoch-%d" % (i_epoch)
            ...
            if (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                ...
                output_model_file = os.path.join(
                    save_dir, "model.{0}.bin".format(i_epoch))
                torch.save(model_to_save.state_dict(), output_model_file)
                output_optim_file = os.path.join(
                    save_dir, "optim.{0}.bin".format(i_epoch))
                torch.save(optimizer.state_dict(), output_optim_file)
                if args.fp16:
                    output_amp_file = os.path.join(
                        save_dir, "amp.{0}.bin".format(i_epoch))
                    torch.save(amp.state_dict(), output_amp_file)
                output_sched_file = os.path.join(
                    save_dir, "sched.{0}.bin".format(i_epoch))
